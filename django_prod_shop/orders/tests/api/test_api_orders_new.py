...
class OrdersAPITest(APITestCase):
    ...
    def check_order_in_order_data(self, order_data, order):
        ...
        self.assertEqual(order_data['adress'], order.adress)
        ...
        self.assertEqual(order_data['yookassa_id'], order.yookassa_id)

    def check_contains_order_in_order_response(self, order_response, order):
        if not hasattr(order_response.data, 'items'):
            self.check_order_in_order_data(order_data=order_response.data, order=order)
            return
        else:
            for order_item in order_response.data['items']:
    ...
    def test_normal_user_can_get_only_his_own_orders_in_orders_list(self):
    ...
    def test_normal_user_can_get_his_own_order_detail(self):
        ...
        self.check_contains_order_in_order_response(
            order_response=detail_normal_response, order=self.order_normal,
        )
    ...
    def test_admin_user_can_get_other_order_detail(self):
        ...
        # Проверка на совпадение заказа из ответа
        self.check_contains_order_in_order_response(
            order_response=detail_admin_response, order=self.order_normal,
        )
